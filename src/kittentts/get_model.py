...
class KittenTTS:
    ...
    def __init__(
        ...
        Args:
            ...
            device: Preferred runtime device ("auto", "cuda", or "cpu")
            providers: Optional explicit ONNX Runtime provider list
        """
        ...
        self.model = download_from_huggingface(
            repo_id=repo_id,
            cache_dir=cache_dir,
            device=device,
            providers=providers,
...
def get_model(repo_id="KittenML/kitten-tts-nano-0.1", cache_dir=None, device="auto", providers=None):
    """Get a KittenTTS model (legacy function for backward compatibility)."""
    return KittenTTS(repo_id, cache_dir, device=device, providers=providers)
